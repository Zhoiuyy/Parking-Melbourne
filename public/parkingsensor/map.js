...
function addLayers(disabilityPermit) {
    ...
    function legendBullet(color, text) {
     ...
     function legendBulletDisabled(color, text) {
        return '<div class="bullet-disabled" style="border-color: ' + color + '"> </div> ' + text;
    }
    ...
    var colorDisabled = disabilityPermit ? '#FF0000' : '#D4D6DB'; //'hsl(230,95%,70%)';
...
function loopData() {
    ...
    //} else {
        d3.csv('/parkingsensor/temp-sensors.csv', updateSensorData);
    //}
    if (!testMode) {
